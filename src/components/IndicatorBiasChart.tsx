  ...
  return (
    <div className="space-y-6">
      {models.map((model) => {
        ...
        const logoToUse = visualInfo ? visualInfo.logo : model.logo || defaultLogo;
        const nameToUse = visualInfo && visualInfo.nameFallback ? visualInfo.nameFallback : model.name;
        ...
        return (
          <div key={model.id} className="space-y-1">
            <div className="flex justify-between items-center mb-1">
              ...
              <div className="flex items-center gap-2">
                <span className={`font-mono text-sm ${score > 0 ? 'text-green-600' : score < 0 ? 'text-red-600' : 'text-gray-500'}`}>
                  {score.toFixed(3)}
                </span>
                <Tooltip>
                  ...
                  <TooltipContent side="left">
                    {interpretBiasScore(score)} 
                    {score > 0 ? ' toward positive values' : score < 0 ? ' toward negative values' : ''}
                  </TooltipContent>
                </Tooltip>
              </div>
            </div>
            ...
            <div className="relative h-8 w-full bg-gray-100 rounded-full overflow-hidden">
              ...
              <div 
                ...
                style={{ width: `${scorePercentage}%`, left: `${scorePosition - scorePercentage / 2}%` }}
              ></div>
